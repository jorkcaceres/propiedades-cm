...
export function databaseError(error:{code?:string;message?:string}):never {
  ...
  if(error.code==='P0001') {
    const allowed=['Primero crea y confirma esta cuenta en Supabase Auth.','El usuario cambió. Actualiza la lista antes de guardar.','No puedes modificar tus propios permisos o estado.','Actualiza el registro antes de guardar.','Este arrendamiento ya tiene pagos; sus condiciones no se pueden sobrescribir.','Selecciona una vivienda y un arrendatario activos.','La solicitud ya fue utilizada con otros datos.','Selecciona un arrendamiento activo.','No puedes registrar un pago recibido en una fecha futura.','El periodo debe estar dentro del arrendamiento.','El pago no está disponible para emitir un recibo.','El pago no está disponible.','Indica el motivo de la anulación.'];
