...
export default async function ActivityPage({searchParams}:{searchParams:Promise<SearchValues>}) {
  ...
  const {data,error,count}=await client.from('pcm_audit_events').select('id,occurred_at,entity,entity_id,action,actor_id,actor_kind',{count:'exact'}).order('occurred_at',{ascending:false}).order('id',{ascending:false}).range(from,to);
  if(error)throw Error('No fue posible consultar la actividad.');
  return <><div className="page-heading"><h1>Actividad</h1><p className="muted">El historial se conserva y no se puede modificar desde la plataforma.</p></div><p className="result-count">{count||0} eventos · Más recientes primero</p><div className="record-list">{!data?.length?<p className="empty-state">No hay eventos en esta página.</p>:data.map(event=><article className="record-card" key={event.id}><h2>{labels[event.action]||event.action} · {labels[event.entity]||event.entity}</h2><p>{new Intl.DateTimeFormat('es-CO',{dateStyle:'medium',timeStyle:'short',timeZone:'America/Bogota'}).format(new Date(event.occurred_at))}</p><p className="small muted">Registro: {event.entity_id}</p><p className="small muted">Responsable: {event.actor_kind==='database_admin'?'Administración de base de datos':event.actor_id}</p></article>)}</div><Pagination page={page} total={count||0} path="/panel/audit"/></>;
}
