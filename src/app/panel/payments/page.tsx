...
export default async function PaymentsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
 ...
 const params=await searchParams;const page=Math.max(1,Math.min(10000,parseInt(params.page||'1')||1));const concept=Object.hasOwn(concepts,params.concept||'')?params.concept!:'all';const status=params.status==='void'?'void':params.status==='all'?'all':'valid';
 let query=client.from('pcm_payments').select('*',{count:'exact'}).order('created_at',{ascending:false}).order('id').range((page-1)*20,page*20-1);if(concept!=='all')query=query.eq('concept',concept);if(status==='valid')query=query.is('voided_at',null);if(status==='void')query=query.not('voided_at','is',null);
 ...
 const link=(n:number)=>`/panel/payments?${new URLSearchParams({page:String(n),concept,status})}`;
 return <><div className="page-heading"><h1>Pagos</h1><p className="muted">Dinero recibido, historial y emisión de comprobantes.</p></div>
  ...
  <nav className="pagination" aria-label="Páginas de pagos">{page>1&&<Link prefetch={false} href={link(page-1)}>Anterior</Link>}{page*20<(result.count||0)&&<Link prefetch={false} href={link(page+1)}>Siguiente</Link>}</nav>
 </>;
}
