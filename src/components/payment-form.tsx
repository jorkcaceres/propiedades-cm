...
export function PaymentForm({leases,payers}:{leases:LeaseRow[];payers:Choice[]}) {
 ...
 async function submit(event:React.FormEvent<HTMLFormElement>){
  ...
  if(!payload){
   try{const data=new FormData(event.currentTarget);payload={request_id:requestId,lease_id:leaseId,amount:Number(amount),payer_name:payer,concept,paid_on:data.get('paid_on'),method:data.get('method'),reference:data.get('reference'),notes:data.get('notes')};}
   catch(e){setError(e instanceof Error?e.message:'Revisa los datos del pago.');return;}
  }
